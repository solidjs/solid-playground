...
export const GridResizer: Component<GridResizerProps> = (props) => {
  const [local, other] = splitProps(props, ['ref', 'class', 'isHorizontal', 'direction', 'onResize']);
  ...
  const directionClasses = () =>
    local.direction === 'horizontal'
      ? `flex-col cursor-col-resize border-l-2 border-r-2 hidden${!local.isHorizontal ? ' md:flex' : ' '}`
  ...
  return (
    <>
      <div
        class={
          isDragging()
            ? `fixed inset-0 z-50 ${local.direction === 'horizontal' ? 'cursor-col-resize' : 'cursor-row-resize'}`
            : 'hidden'
        }
      />
